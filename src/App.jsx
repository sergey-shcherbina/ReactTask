      ...
      <div>
        <label>
          <input 
            ...
            onChange={() => setChecked(dispatch({type: 'CHECKED_OFF', payload: employee}))}
          /> not active
        </label>
        <br />
        <label>
          <input 
            ...
            onChange={() => setChecked(dispatch({type: 'CHECKED_ON', payload: employee}))}
          /> active
        </label>
      </div>
    )
  ...
  const employeesOutput = arr.map(elem => {
    return <section className="wrapper-left" key={elem[0].charCodeAt(0)}>
      ...
      {elem.slice(1).map(employee => {
        // {employee.active === 'active' ? activeColor = 'blue' : activeColor = 'black'}
